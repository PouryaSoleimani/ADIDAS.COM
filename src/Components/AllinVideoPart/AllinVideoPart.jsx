...
const AllinVideoPart = () => {
  ...
  const vidRef = useRef();
  const vidPause = () => { vidRef.current.pause() }
  const vidPlay = () => { vidRef.current.play() }
  return (
    < >
      <video autoPlay loop muted disablePictureInPicture src='/videos/adidas, all in.mp4' ref={vidRef} className='w-full relative z-0' />
      {/* NEW COLLECTION ===============================================================================================================*/}
      ...
      {/* PLAY AND PAUSE */}
        <div className='flex space-x-2'>
          ...
          <button onClick={vidPause} className='md:px-4 md:p-4 rounded-full duration-300 hover:bg-black hover:text-white flex items-center justify-center'><FaPause className='text-white w-5 h-5' /></button>
        </div>
      </div>
    </>
  )
